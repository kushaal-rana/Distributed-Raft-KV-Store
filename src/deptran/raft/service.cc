...
namespace janus
{
  ...
  RaftServiceImpl::RaftServiceImpl(TxLogServer *sched) : svr_((RaftServer *)sched)
  {
<<<<<<< HEAD
                                            ...
                                            rrr::DeferredReply *defer)
  {
    ...
    else
    {
      ...
      if (prefixLength > svr_->commands.size() || (prefixLength > 0 && prefixLogTerm != svr_->terms[prefixLength - 1]))
      {
        *matchedIndex = 0;
        *success = false;
      }
                                        ...
                                        rrr::DeferredReply *defer)
  {
    ...
    *retTerm = svr_->currentTerm;
    svr_->m.unlock();
=======
    srand(time(NULL) + svr_->loc_id_);
  }

  void RaftServiceImpl::HandleRequestVote(const uint64_t &termCandidate,
                                          const uint64_t &candidateId,
                                          uint64_t *retTerm,
                                          bool_t *voteGranted,
                                          rrr::DeferredReply *defer)
  {
    svr_->ProcessRequestVote(termCandidate, candidateId, *retTerm, *voteGranted);
    defer->reply();
  }
                                            ...
                                            rrr::DeferredReply *defer)
  {
    ...
    if (!cmd)
    {
      // Reset follower's election timeout
      svr_->ResetElectionDeadline();
      // Append the command to the log
      *followerAppendOK = true;
    }
    else
    {
      // Log replication case: attempt to append the new log entry.
      bool appended = svr_->AppendLogEntry(cmd);
      *followerAppendOK = appended;
    }
